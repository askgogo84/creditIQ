...
async function cashSmoke(date: string) {
  if (skyscannerFlightsConfigured()) {
    try {
      ...
      if (result.flights.length) return { source: 'skyscanner-live', rows: result.flights.length, verified: true }
    } catch {}
  }
  if (amadeusFlightsConfigured()) {
    try {
      const result = await searchAmadeusFlights({ from: 'BLR', to: 'BOM', date, cabin: 'economy', adults: 1, max: 20 })
      if (result.flights.length) return { source: 'amadeus', rows: result.flights.length, verified: true }
    } catch {}
  }
  const token = process.env.TRAVELPAYOUTS_TOKEN || ''
  if (!token) return { source: 'none', rows: 0, verified: false }
  try {
    ...
    return { source: 'travelpayouts-v3', rows, verified: false, httpStatus: response.status }
  } catch {
    return { source: 'travelpayouts-v3', rows: 0, verified: false }
  }
}
...
async function main() {
  ...
  const awardtoolAttempted = awards.some(item => item.attempts.some(attempt => attempt.source === 'awardtool' && attempt.configured))
  const awardtoolErrorsOnly = awards.every(item => item.attempts.some(attempt => attempt.source === 'awardtool' && attempt.state === 'ERROR'))
  const awardToolOperational = awardtoolAttempted && !awardtoolErrorsOnly
  ...
  const cashOperational = cash.rows > 0
  ...
  console.log(`TRAVEL_SMOKE_RESULT=${JSON.stringify(result)}`)
  if (!awardToolOperational || !cashOperational || !hotelOperational || !walletOperational) process.exit(1)
}

main().catch(error => {
  console.error(`TRAVEL_SMOKE_FATAL=${error instanceof Error ? error.message : 'unknown error'}`)
  process.exit(1)
})
