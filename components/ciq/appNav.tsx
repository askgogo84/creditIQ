...
//   - components/ciq/AppRail.tsx  â€” desktop left rail (>=900px)
//   - components/ciq/TabBar.tsx   â€” mobile bottom bar (<900px)
//   - components/Header.tsx       â€” signed-in top nav on pages not yet migrated
//                                    into the (shell) route group
//
// IA: docs/00-SIGNED-IN-IA.md Â§2/Â§3 cuts the signed-in nav from ~26 destinations
...
// reachable â€” no orphaned pages, no 404s. The grouped secondary directory
...
// has no surface yet â€” it will grow out of the current /dashboard wallet and split
...
// then its bound routes (/feed, /intelligence) fold under Wallet â€” the /dashboard
...
export type AppNavItem = {
  ...
  label: string
  href: string
  // A self-contained lucide-react icon component â€” the SINGLE source every nav
  ...
  Icon: LucideIcon
  // Which paths fold into this destination so the tab stays lit â€” marketing
...
export const APP_NAV: AppNavItem[] = [
  {
    ...
    Icon: Wallet,
    // Holdings ledger â€” the real, shipped wallet (WalletView / HeroGauge / CardRow /
    // 2-step tour). Folds: /statement-truth (verification happens inside Wallet, IA Â§3),
    // and /feed + /intelligence (Feed â†’ Home's "what changed" column; Home unbuilt, so
    // these fold onto the /dashboard surface they'll grow from â€” move to Home when it
    // ships). (/my-cards retired â€” it was an unreachable legacy dup of /dashboard.)
  ...
  },
  {
    ...
    // pending the Spend rebuild (IA Â§3).
  ...
  },
  {
    ...
    // Trip Planner = tabs & modes within Travel (IA Â§3).
  ...
  },
  {
    ...
    // card stays a link, not a route match â€” IA Â§3).
...
// Icon: a lucide-react component â€” the SAME vocabulary APP_NAV uses, so the section strip
...
// In-page ("section") navigation â€” the tabs rendered INSIDE each destination so the
// features folded into it (IA Â§3) stay one click away. This is the entry point that
...
// never point at a page that renders its own marketing <Header> â€” that drops the user
...
//   /flights, /best-cards/*, /uae, /smart-match  â€” render the marketing Header.
//   /optimize                                     â€” superseded redemption tool.
