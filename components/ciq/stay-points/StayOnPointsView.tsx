...
function pathLabel(card: StayCard): string {
  if (transferExecutionBlocked(card)) {
    return 'Best-ranked path: transfer — verification needed';
  }
  return PATH_COPY[card.recommended_path].label;
}

export default function StayOnPointsView(p: Props) {
  const [open, setOpen] = useState<string | null>(null);

  const executable = p.cards.filter(
    (c) => c.recommended_path !== 'NO_RECOMMENDATION' && !transferExecutionBlocked(c),
  ).length;
  const transferPathsBlocked = p.cards.filter(transferExecutionBlocked).length;

  const countCopy =
    p.balance === null
      ? `${p.cards.length} hotels`
      : transferPathsBlocked > 0
        ? `${executable} executable · ${transferPathsBlocked} transfer paths need verification`
  ...
  return (
    <div className="sop" data-mode={p.mode}>
        ...
        <div className="bandTop">
          <div>
            ...
            <div className="bandValue">
              {p.programmeConversionValueInr !== null
                ? `${money(p.programmeConversionValueInr)} / HDFC Reward Point`
                : 'FX comparison unavailable'}
            </div>
          </div>
      ...
      <div className="barline">
        ...
        <div className="count">{countCopy}</div>
      </div>
      ...
      {p.cards.map((c) => {
        ...
        const guardedTransfer = transferExecutionBlocked(c);
        const tone = guardedTransfer ? 'guarded' : path.tone;
        ...
        return (
          <article key={c.id} className="card">
            <div className="cardBody">
              ...
              <div className="priceRow">
                ...
                {c.execution_cash_payable_inr !== null && c.recommended_path === 'TRANSFER_THEN_BOOK' && (
                  <div className="rightPrice">
                    <div className="k">
                      {guardedTransfer && c.programme_points_spent !== null
                        ? `Projected after ${pts(c.programme_points_spent)} ALL`
                        : 'After selected ALL block'}
                    </div>
                    <div className={`price ${guardedTransfer ? 'guardedValue' : 'goodText'}`}>
                      {inr(c.execution_cash_payable_inr)}
                    </div>
              ...
              <section className={`path path-${tone}`}>
                <div className="pathLabel">
                  {p.balance === null ? 'Add your balance to unlock this path' : pathLabel(c)}
                </div>
                {p.balance === null ? (
                  ...
                  <>
                    {guardedTransfer && (
                      <div className="guardedIntro">
                        The ranking is stable on the facts we have, but this is not yet an executable transfer instruction.
                      </div>
                    )}
              ...
              <div className="actions">
                ...
                <a className="btn secondary" href={c.booking_url} target="_blank" rel="noopener noreferrer">
                  Check direct
                </a>
              </div>
...
function PathSteps({ card: c, balance }: { card: StayCard; balance: number }) {
  if (c.recommended_path === 'TRANSFER_THEN_BOOK') {
    ...
    const exactTransferKnown = c.bank_points_exact !== null;
    ...
    return (
      <div className="steps">
        ...
        <Step
          ...
          title={
            exactTransferKnown
              ? `Transfer exactly ${pts(c.bank_points_exact as number)} HDFC Reward Points`
              : 'Do not transfer yet — exact issuer step is withheld'
          }
        >
          {c.programme_points_spent !== null && c.bank_points_target !== null && (
            <>
              ...
              {' '}
              {enoughForTarget
                ? 'Your current balance reaches that arithmetic target.'
                : `Your current balance is ${pts(c.bank_points_target - balance)} HDFC Reward Points short of that target.`}
            </>
          )}
          {!exactTransferKnown && <Blockers card={c} />}
        </Step>

        <Step
          n="3"
          title={
            c.programme_points_spent !== null
              ? `${exactTransferKnown ? 'Redeem' : 'If verified, redeem'} ${pts(c.programme_points_spent)} ${c.programme_name} points`
              : 'Use the verified programme block'
          }
        >
          {c.points_offset_inr !== null && c.execution_cash_payable_inr !== null ? (
            <>
              {exactTransferKnown ? 'That block offsets ' : 'At today’s live FX, that candidate would offset '}
              <b>{inr(c.points_offset_inr)}</b> on this booking, leaving <b>{inr(c.execution_cash_payable_inr)}</b> in cash.
            </>
          ) : (
  ...
  if (c.recommended_path === 'REDEEM_EXISTING_BALANCE') {
    return (
      <div className="steps">
        <Step n="1" title="Use the programme points you already hold">
          No card-points transfer is required.
        </Step>
        <Step n="2" title={`Book direct with ${c.programme_name}`}>
          {c.programme_points_spent !== null
            ? `Redeem ${pts(c.programme_points_spent)} ${c.programme_name} points.`
            : 'Use the verified redemption amount.'}
        </Step>
      </div>
    );
  }
    ...
    return (
      <div className="steps">
        <Step n="1" title="Use SmartBuy instead of transferring">
          {c.portal_points_used !== null
            ? <>Use <b>{pts(c.portal_points_used)} HDFC Reward Points</b> through SmartBuy.</>
            : 'Use the portal path shown by the engine.'}
        </Step>
        <Step n="2" title="Pay the remaining cash">
          {c.portal_cash_payable_inr !== null
            ? <>Cash payable is <b>{inr(c.portal_cash_payable_inr)}</b>, including the redemption fee.</>
            : 'Cash remainder unavailable.'}
        </Step>
      </div>
    );
  }
  ...
  if (c.recommended_path === 'CASH_AND_RETAIN') {
    return (
      <div className="pathBody">
        Pay {inr(c.cash_total_inr)} in cash and retain your HDFC Reward Points. The portal fee/cap makes redeeming now worse on this booking.
      </div>
    );
  }
  ...
  if (c.recommended_path === 'QUOTE_REQUIRED') {
    return (
      <div className="pathBody">
        This programme uses award pricing that must be checked for the exact stay. CreditIQ will not manufacture a points price.
      </div>
    );
  }
  ...
  return (
    <div className="pathBody">
      ...
      {c.balance_state === 'BELOW_MINIMUM' && (
        <div className="smallGap">Your current balance cannot reach the smallest permitted redemption candidate.</div>
      )}
    </div>
  );
}
...
function Blockers({ card: c }: { card: StayCard }) {
  ...
  if (c.transfer_state === 'RATIO_ONLY') {
    items.push('HDFC transfer minimum/increment is not yet sourced.');
  }
  ...
  if (c.rule_state === 'SOURCE_CONFLICT') {
    items.push('Accor’s 1,000-point online floor is disputed; CreditIQ is using only the common 2,000-point blocks.');
  }
  if (c.instruction_blocked === 'RATIO_SOURCE_CONFLICT') {
    items.push('The transfer ratio itself is disputed, so no transfer arithmetic is actionable.');
  }
  if (items.length === 0) {
    items.push('An execution-critical input is not verified.');
  }
...
function Evidence({ card: c, props: p }: { card: StayCard; props: Props }) {
  return (
    <div className="evidence">
      ...
      {c.programme_points_spent !== null && (
        <Row
          k={`${c.programme_name} points selected`}
          v={pts(c.programme_points_spent)}
          s="legal redemption candidate from the v3.1 engine"
        />
      )}

      {c.bank_points_target !== null && (
        <Row
          k="HDFC Reward Points arithmetic target"
          v={pts(c.bank_points_target)}
          s={c.bank_points_exact === null ? 'not an exact transfer instruction' : 'issuer min/increment verified'}
        />
      )}

      {c.bank_points_exact !== null && (
        <Row
          k="Exact HDFC transfer"
          v={pts(c.bank_points_exact)}
          s="after issuer minimum/increment rounding"
        />
      )}

      {c.points_offset_inr !== null && (
        <Row k="Booking offset" v={inr(c.points_offset_inr)} s="booking-specific result" />
      )}
      {c.execution_cash_payable_inr !== null && (
        <Row
          k="Cash after programme redemption"
          v={inr(c.execution_cash_payable_inr)}
          s="separate from transfer affordability"
        />
      )}
      {c.portal_points_used !== null && (
        <Row k="SmartBuy points used" v={pts(c.portal_points_used)} s={`portal cap ${p.portalCapPct}%`} />
      )}
      {c.portal_cash_payable_inr !== null && (
        <Row
          k="SmartBuy cash payable"
          v={inr(c.portal_cash_payable_inr)}
          s={`includes ${money(c.portal_fee_inr ?? p.portalFeeInr)} redemption fee`}
        />
      )}
      {c.conversion_value_per_bank_point_inr !== null && (
        <Row
          k="Programme conversion value"
          v={`${money(c.conversion_value_per_bank_point_inr)} / HDFC Reward Point`}
          s="programme-level; not booking-specific"
        />
      )}
      {c.booking_specific_value_per_bank_point_inr !== null && (
        <Row
          k="Booking-specific unlocked offset"
          v={`${money(c.booking_specific_value_per_bank_point_inr)} / transferred HDFC point`}
          s="may be higher when an existing programme balance is unlocked; not intrinsic point value"
        />
      )}
      {p.fx && <Row k="EUR / INR" v={p.fx.rate.toFixed(2)} s={`live · ${p.fx.source}`} />}
      <Row
        k="SmartBuy nominal value"
        v={`${money(p.portalPerPoint)} / HDFC Reward Point`}
...
@media(min-width:1100px){.sop{max-width:820px}}
...
.priceRow{display:flex;justify-content:space-between;gap:16px;margin-top:14px;padding:12px 0;border-top:1px solid #f0ede7;border-bottom:1px solid #f0ede7}.price{font-size:20px;font-weight:850;margin-top:2px}.rightPrice{text-align:right}.goodText{color:var(--good)}.guardedValue{color:var(--warn)}
.path{margin-top:12px;border-radius:13px;padding:12px;border:1px solid}.path-good{background:var(--goodBg);border-color:var(--goodLine)}.path-neutral{background:#f5f6f8;border-color:#e1e4e9}.path-warn{background:var(--warnBg);border-color:var(--warnLine)}.path-guarded{background:#fff8ec;border-color:#e6c990}.path-unknown{background:var(--unkBg);border-color:var(--unkLine)}.pathLabel{font-size:11px;letter-spacing:.05em;text-transform:uppercase;font-weight:850;margin-bottom:10px}.path-good .pathLabel{color:var(--good)}.path-warn .pathLabel,.path-guarded .pathLabel{color:var(--warn)}.path-unknown .pathLabel{color:var(--unk)}.pathBody{font-size:13px;line-height:1.55;color:var(--ink2)}.guardedIntro{font-size:12px;line-height:1.5;color:#725526;background:rgba(255,255,255,.58);border-radius:9px;padding:8px 9px;margin:-1px 0 11px}.smallGap{margin-top:5px;font-weight:700}
...
@media(max-width:520px){.barline{align-items:flex-start}.count{max-width:55%}.priceRow{gap:10px}.bandTop{align-items:flex-start}.sourceBadge{font-size:9px}}
`;