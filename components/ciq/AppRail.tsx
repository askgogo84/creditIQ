...
// the approved v3 prototype's dark editorial rail. Gated on auth state (NavShell),
...
const navRow = (active: boolean): React.CSSProperties => ({
  ...
  background: active ? 'linear-gradient(90deg, rgba(189,145,61,.21), rgba(189,145,61,.035))' : 'transparent',
  ...
  return (
    <aside
      ...
      style={{
        ...
        background: 'radial-gradient(circle at 14% 12%, rgba(189,145,61,.10), transparent 14rem), linear-gradient(180deg, #151613, #10110f 64%, #0c0d0b)', borderRight: '1px solid #2d3029',
    ...
    >
      ...
      <Link href="/dashboard" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', padding: '18px 18px 16px', flexShrink: 0 }}>
        <div style={{ width: 36, height: 36, background: 'linear-gradient(145deg, #d7b76f, #bd913d)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 10px 24px rgba(189,145,61,.16)' }}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
            <rect x="1" y="4" width="22" height="16" rx="3" fill="#1b1c18" fillOpacity="0.96" />
            <line x1="1" y1="10" x2="23" y2="10" stroke="#d7b76f" strokeWidth="2" />
            <circle cx="5" cy="14.5" r="1.5" fill="white" />
          </svg>
        </div>
        <div>
          <div style={{ fontFamily: 'var(--font-serif)', fontSize: 18, fontWeight: 600, color: '#fff', letterSpacing: '-0.5px', lineHeight: 1 }}>CreditIQ</div>
        ...
        {APP_NAV.filter(item => item.key !== 'you').map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} style={navRow(active)}>
              ...
              <Icon size={20} strokeWidth={1.8} color={active ? '#bd913d' : 'currentColor'} style={{ flexShrink: 0 }} aria-hidden />
