...
export function buildFlightConciergeRequest(
  ...
  return {
    ...
    selection: {
      ...
      cabin: award?.cabin ?? row.cashCabin ?? null,
      programme: award?.program ?? null,
      award_source: award?.source ?? null,
      mileage_cost: award?.mileageCost ?? null,
      seats: award?.seats ?? null,
      flight_numbers: trip?.flightNumbers ?? row.flightNumbers ?? null,
      carriers: trip?.carriers ?? row.airlines ?? row.airline ?? null,
      ...
      booking_link: row.bookingLink ?? row.booking_link ?? row.deep_link ?? null,
    ...
    sourceSnapshot: {
      ...
      cash: {
        source: decision?.sourceAuthority.cash ?? row.provider ?? row.source ?? null,
        state: row.price > 0 ? 'PROVIDER_RETURNED' : 'UNAVAILABLE',
        booking_link_present: Boolean(row.bookingLink ?? row.booking_link ?? row.deep_link),
      },
      transfer_candidates: {
