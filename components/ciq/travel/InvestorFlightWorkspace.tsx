...
export function InvestorFlightWorkspace() {
  ...
  const search = async () => {
    ...
    try {
      ...
      setRows(((data.flights || []) as FusionRow[]).filter((row) => row.award))
    } catch {
    ...
    <div className="ifw-root">
      <div className="ifw-title-row">
        <div>
          ...
          <p className="ifw-sub">Award inventory on the left. Every supported card in your wallet compared on the right. Unsupported or incomplete cards stay visible instead of disappearing.</p>
        </div>
        ...
        <div className="ifw-workspace">
          <div className="ifw-left">
            <div className="ifw-toolbar">
              <div><strong>{filtered.length} option{filtered.length === 1 ? '' : 's'}</strong><span>{labelFor(from)} → {labelFor(to)} · {fmtDate(date)} · {cabin}</span></div>
            ...
            {filtered.length === 0 ? (
              <div className="ifw-empty">
                {rows.length === 0 ? 'No award seats found for this route and date.' : cardsScope === 'mine' ? <>Award seats exist, but none have a known route from the cards in your wallet. <button onClick={() => setCardsScope('all')}>Show all award seats</button>.</> : 'No result matches the current filters.'}
              </div>
            ) : (
              <div className="ifw-list" role="listbox" aria-label="Award flight options">
                {filtered.map((row) => <FlightResultRow key={row.id} row={row} active={row.id === selectedId} onSelect={() => setSelectedId(row.id)} />)}
              </div>
            )}
          </div>
...
function FlightResultRow({ row, active, onSelect }: { row: FusionRow; active: boolean; onSelect: () => void }) {
  ...
  return (
    <button className={`ifw-row${active ? ' active' : ''}`} onClick={onSelect} role="option" aria-selected={active}>
      ...
      <div className="ifw-programme"><b>{award.program}</b><span>{row.from} → {row.to} · {fmtDuration(trip?.durationMinutes || row.duration * 60)} · {fmtStops(stops)}</span><small>{award.source ? 'Award source available' : 'Award source unknown'}</small></div>
      <div className="ifw-metric"><b>{award.mileageCost.toLocaleString('en-IN')}</b><span>{award.program} miles</span>{taxes && <small>+ {taxes} taxes</small>}</div>
      ...
      <div className="ifw-best"><b>{best ? best.cardName : 'No route'}</b><span>{best?.cardPointsNeeded != null ? `${best.cardPointsNeeded.toLocaleString('en-IN')} card pts` : 'not priced'}</span></div>
    </button>
  )
}

function FlightDecisionPanel({ row }: { row: FusionRow | null }) {
  if (!row || !row.award) return <aside className="ifw-panel empty"><div><b>Select a flight</b><p>CreditIQ will compare every card returned by your wallet for that award programme.</p></div></aside>
  ...
  return (
    <aside className="ifw-panel">
      <div className="ifw-panel-head">
        ...
        <p>Compared across {options.length} card{options.length === 1 ? '' : 's'} in this wallet.{supportedCount < options.length ? ` ${options.length - supportedCount} card${options.length - supportedCount === 1 ? '' : 's'} need mapping or have no known route.` : ''}</p>
        <div className="ifw-award-cost"><b>{award.mileageCost.toLocaleString('en-IN')} {award.program} miles</b><span>{taxes ? `+ ${taxes} taxes` : 'taxes not supplied'}</span></div>
      </div>

      <div className="ifw-compare">
        <div className="ifw-section-label">Compared across your cards</div>
        {options.length === 0 ? <p className="ifw-muted">No cards were returned by the wallet for this search.</p> : options.map((option) => {
          const isBest = sameWalletOption(option, best)
          return (
            <div key={`${option.bank}|${option.cardName}`} className={`ifw-card-option${isBest ? ' winner' : ''}`}>
              <div><b>{option.cardName}</b><span>{option.bank}</span><small>{walletOptionReason(option, award.program)}</small></div>
              <div className="ifw-option-right">
                {option.status === 'ok' && option.cardPointsNeeded != null ? <><b>{option.cardPointsNeeded.toLocaleString('en-IN')}</b><span>card points</span><small>{option.selfEntered ? 'Self-entered balance' : 'Wallet balance'}</small></> : <><b>—</b><span>{option.status === 'currency-unknown' ? 'not mapped' : 'no route'}</span></>}
              </div>
      ...
      <div className="ifw-cash-compare">
        ...
        {row.price > 0 ? <div className="ifw-cash-row"><span>Same search cash fare</span><b>₹{row.price.toLocaleString('en-IN')}</b></div> : <p className="ifw-muted">No matched cash fare was returned for this award row. CreditIQ is not inventing one.</p>}
...
function DecisionStep({ n, title, text }: { n: string; title: string; text: string }) {
  return <div className="ifw-step"><div>{n}</div><p><b>{title}</b><span>{text}</span></p></div>
}
