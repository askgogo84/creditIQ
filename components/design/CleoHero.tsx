...
const SCENES = {
  roast: { label: 'Roast my card', emoji: '🔥', accent: '#C46A52', bg: 'rgba(196,106,82,0.10)', intro: "Drop your card. I'll tell you the truth.", placeholder: 'Try: "I have SBI SimplyCLICK"', messages: [{ role: 'user', text: 'I have HDFC Regalia. ₹2,500 fee.' },{ role: 'bot', text: 'Oof. Sweetie.' },{ role: 'bot', text: "Your Regalia earns 1.4% effective.\n\nOn ₹8L/yr spend: ₹11,200 back.\nHDFC Infinia: ₹26,400 back.\n\nGap: ₹15,200/yr you're handing back." },{ role: 'bot', text: 'Grade: D−' },{ role: 'user', text: 'What should I switch to?' },{ role: 'bot', text: "Tell me your monthly spend and I'll match you. ✨" }] },
  match: { label: 'Find my card', emoji: '🎯', accent: '#D89B2A', bg: 'rgba(212,163,115,0.12)', intro: "Tell me how you spend. I'll pick one card.", placeholder: 'Try: "₹8L spend, 50% online"', messages: [{ role: 'user', text: '₹12L/yr. Mostly travel, online, dining.' },{ role: 'bot', text: 'OK travel girlie. Working on it...' },{ role: 'bot', text: "Top match: HDFC Infinia (94/100)\n\n• 3.3% effective on your profile\n• Unlimited Priority Pass\n• ₹47,000/yr back after fees\n\nBreak-even: ₹6.2L. You're way past it." },{ role: 'user', text: 'What about Magnus?' },{ role: 'bot', text: 'Magnus earns ₹19k less. Recent devaluation crushed it. Skip.' }] },
  optimize: { label: 'Use my points', emoji: '✨', accent: '#7C8970', bg: 'rgba(124,137,112,0.14)', intro: "Got points sitting there? Let's use them.", placeholder: 'Try: "I have 80k Axis Edge points"', messages: [{ role: 'user', text: 'I have 150,000 HDFC points.' },{ role: 'bot', text: 'Excellent. Time to make them work.' },{ role: 'bot', text: "Best path:\n\n→ Transfer to Turkish Miles&Smiles (1:1)\n→ Book DEL-IST-NRT in business\n→ Cost: 90,000 miles + ₹18,000 taxes\n→ Cash price: ₹3,40,000\n\nValue: ₹5.30 per point. Elite." },{ role: 'user', text: 'Business class?? With 90k miles??' },{ role: 'bot', text: 'Welcome to the sweet spot 🌶' }] },
} as const
...
function Bubble({ role, text, delay=0 }: { role:string; text:string; delay?:number }) {
  ...
  useEffect(() => { if(delay===0) return; const t=setTimeout(()=>setOn(true),delay); return ()=>clearTimeout(t) },[delay])
  if(!on) return null
  const u = role==='user'
  return <div style={{display:'flex',justifyContent:u?'flex-end':'flex-start',marginBottom:6,animation:'bIn 0.3s ease'}}>
    <div style={{maxWidth:'82%',padding:'9px 13px',borderRadius:u?'16px 16px 3px 16px':'16px 16px 16px 3px',background:u?'#142950':'#F4EFE7',color:u?'#FFF':'#142950',fontSize:13.5,lineHeight:1.4,whiteSpace:'pre-wrap'}}>{text}</div>
  </div>
}
...
function Dots({ delay=0 }: { delay?:number }) {
  ...
  if(!on) return null
  return <div style={{display:'flex',marginBottom:6}}><div style={{padding:'10px 13px',background:'#F4EFE7',borderRadius:'16px 16px 16px 3px',display:'flex',gap:4}}>
...
function Phone({ scene, msgs }: { scene:SceneKey; msgs:Msg[] }) {
  const d=SCENES[scene], ref=useRef<HTMLDivElement>(null)
  useEffect(()=>{ const t=setInterval(()=>ref.current?.scrollTo({top:99999,behavior:'smooth'}),1200); return ()=>clearInterval(t) },[scene,msgs])
  return <div style={{width:'100%',height:'100%',background:'linear-gradient(180deg,#FBF7F0,#F2EAD8)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
...
export function CleoHero() {
  ...
  useEffect(()=>{ if(sent) return; setMsgs(SCENES[mode].messages.map((m,i)=>({...m,delay:i*1100}))) },[mode,sent])
  useEffect(()=>{ if(sent) return; const keys=Object.keys(SCENES) as SceneKey[]; const t=setTimeout(()=>setMode(k=>keys[(keys.indexOf(k)+1)%keys.length]),9000); return ()=>clearTimeout(t) },[mode,sent])

  const send = async () => {
    if(!input.trim()||loading) return
    const text=input.trim(); setInput(''); setLoading(true); setSent(true)
    setMsgs([{role:'user',text,delay:0},{role:'bot',text:'...',delay:600}])
    try {
      const r=await fetch('/api/travel-ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,history:[]})})
      const d=await r.json()
      setMsgs([{role:'user',text,delay:0},{role:'bot',text:d.reply||'Try the full tool for a detailed answer.',delay:0}])
    } catch { setMsgs([{role:'user',text,delay:0},{role:'bot',text:"Couldn't reach the engine. Try the full Roast tool 👉",delay:0}]) }
        ...
        <div style={{display:'grid',gridTemplateColumns:'minmax(0,1.1fr) minmax(0,0.9fr)',gap:'clamp(40px,6vw,80px)',alignItems:'center'}} className="grid-1-mobile">
          <div>
            ...
            <p style={{fontSize:'clamp(15px,1.3vw,18px)',color:'var(--ink-2,#2A3F6B)',maxWidth:520,lineHeight:1.55,marginBottom:32}}>
              Compare 100+ cards with{' '}<span style={{fontFamily:'var(--font-serif,Georgia,serif)',fontStyle:'italic',color:'var(--ink,#142950)'}}>zero affiliate bias</span>.{' '}
            ...
            <div style={{display:'flex',gap:8,alignItems:'center',padding:'6px 6px 6px 18px',background:'var(--surface,#fff)',border:'1px solid var(--line-strong,rgba(20,41,80,0.2))',borderRadius:999,boxShadow:'0 4px 14px rgba(20,41,80,0.07)',maxWidth:520}}>
              <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()}
                placeholder={d.placeholder} disabled={loading}
              ...
              <button onClick={send} disabled={loading||!input.trim()}
                style={{padding:'12px 20px',borderRadius:999,background:'var(--ink,#142950)',color:'var(--bg,#F5EFE6)',border:'none',fontSize:14,fontWeight:600,cursor:loading||!input.trim()?'not-allowed':'pointer',opacity:loading||!input.trim()?0.5:1,whiteSpace:'nowrap'}}>
                {loading?'...':'Ask →'}
              </button>
            </div>
