    ...
    <div style={{ background: 'var(--surface,#F8F9FC)', borderTop: '1px solid var(--line,rgba(20,41,80,0.08))', padding: 'clamp(32px,5vw,56px) 0' }}>
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 clamp(16px,4vw,40px)' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--copper,#8C5F12)', letterSpacing: '0.14em', marginBottom: 8 }}>REWARDS CALCULATOR</div>
          <h2 style={{ fontSize: 'clamp(22px,3vw,32px)', fontWeight: 800, color: 'var(--ink,#142950)', margin: '0 0 8px', lineHeight: 1.2 }}>Are you leaving money on the table?</h2>
          <p style={{ fontSize: 15, color: 'var(--ink-3,#5A6A8A)', margin: 0 }}>Pick your card and monthly spend. See what you're missing in 5 seconds.</p>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 20, padding: 'clamp(20px,4vw,32px)', boxShadow: '0 4px 24px rgba(20,41,80,0.10)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 12, marginBottom: 20 }}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-3,#5A6A8A)', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>YOUR BANK</label>
              <select value={selectedBank} onChange={e => setSelectedBank(e.target.value)}
                style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid var(--line,rgba(20,41,80,0.15))', fontSize: 14, color: 'var(--ink,#142950)', background: 'white' }}>
                {banks.map(b => <option key={b} value={b}>{b}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-3,#5A6A8A)', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>YOUR CARD</label>
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--ink-3,#5A6A8A)', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>MONTHLY SPEND</label>
          ...
          <button onClick={calculate} disabled={!selectedCard || loading}
            style={{ width: '100%', padding: '14px', borderRadius: 10, background: 'var(--navy,#1B3A5C)', color: 'white', fontSize: 15, fontWeight: 700, border: 'none', cursor: 'pointer', marginBottom: result ? 20 : 0 }}>
          ...
          {result && !result.is_best && result.annual_gap > 0 && (
            <div style={{ borderTop: '1px solid var(--line,rgba(20,41,80,0.08))', paddingTop: 20 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <button onClick={() => router.push('/rewards-calculator')}
          ...
          {result && result.is_best && (
            <div style={{ borderTop: '1px solid var(--line,rgba(20,41,80,0.08))', paddingTop: 20, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 14, color: 'var(--ink-3,#5A6A8A)', marginTop: 8 }}>Earning {fmt(result.current_card.rewards_monthly)}/month in rewards.</div>
            </div>
          )}
        </div>
      </div>
