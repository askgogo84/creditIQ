...
## BATCH 2 — Small independent bugs ✅ COMPLETE (12 Aug)

Unrelated to each other and to the page pass. All self-contained. **Outcome:** #7 prefetch test, #8 `/landing` 301, #11 SECTION_TABS→lucide, #14 dead-file sweep — shipped. #5 nav-auth — mitigated + shipped. #6 count-up stall — fixed + shipped. #9 `/optimize` and #10 `reassertTheme` — dropped (wrong premises; both load-bearing, kept). #12 `/login` video — already done. #13 Reveal — report-only, its own change. #15 font — **not separable; moved to the Batch 3/4 gold walk** (see below). Nothing left in Batch 2.

5. **Nav auth resolution — MITIGATED (shipped 12 Aug, `ee208d02`).** `NavShell` used `!user` (true for both loading `undefined` and signed-out `null`), so a signed-in user saw the marketing `<Header/>` until `getSession()` resolved. A full fix would need dynamic rendering of the whole `(shell)` group (regressing the static/ISR public catalogue) — not worth it. **Mitigation:** a client **layout** effect reads the Supabase auth cookie synchronously (post-hydration, pre-paint) and flips to the shell immediately, skipping the `getSession()` round-trip. The read is in an effect, never the render body → first client render still equals SSR (`undefined`→Header), so no hydration mismatch / `data-theme` strip; the group stays static/ISR. **Measured on preview: wrong-bar window ~329ms → ~9ms.** ⚠ Caveat: the 329ms baseline came from a **fake token**, which forces `getSession()`'s refresh network round-trip; a user with a still-valid token was already fast. The real win is **expired-token return visits** — the case that produced the persistent wrong bar on a real phone. `?flashdebug` logs the before/after timings (gated, inert otherwise).
6. **Wallet count-up stall — FIXED (12 Aug, `c24d1da6`).** `HeroGauge` headline + bar fill now default to the true value (`useState(target)` / `fill=true`), with the count-up/grow-in as layout-effect progressive enhancement and a `setTimeout` guard that snaps to truth if rAF never completes (backgrounded tab / throttle / dropped frame). Regression test covers the rAF-never-fires guard path. The production 3,68,844-vs-8,76,499 incident can no longer occur. **Follow-up:** the gauge bar still animates `width` (impeccable `layout-transition` flag). Converting it to `transform: scaleX` is **available to test on `/dashboard`** — the pinned-transform bug is documented only on the marketing surface (`page.module.css`), and nothing has shown transform pins on `/dashboard`, so test-then-convert, don't defer by assuming the marketing constraint applies.
...
10. **`reassertTheme` — KEEP (dropped from Batch 2, 12 Aug).** The "redundant effect" diagnosis is **wrong**. `reassertTheme()` in `Header.tsx:96` and `NavShell.tsx:62` is the load-bearing fix for React **stripping the pre-paint `data-theme` during hydration** on server-rendered routes — signed-in `/card/[slug]` renders no `<Header>`, so `NavShell` is the only thing that restores it there. It's guarded (`lib/store.ts:94` — no-op if the attr survived), so it never flashes, and the pre-paint alone cannot help post-hydration. Removing it reintroduces the theme desync. **Not removed.**
11. **Icon vocabulary.** `SECTION_TABS` still runs its own path-icon factory while the nav is on lucide. One vocabulary.
12. **`/login` video weight — ALREADY DONE (stale entry; confirmed 12 Aug).** `/login` was rebuilt with a poster-first media gate: the `<video>` mounts only on desktop ≥768px with motion allowed (`showVideo`, `app/login/page.tsx`), so phones and reduced-motion users get poster-only and never fetch the clip. The old `auth-bg-mobile.*` clip is no longer referenced (login now uses `/video/signin-bg.*`). Remaining: the now-dead `auth-bg*` asset files → deleted in #14.
13. **Reveal — same animation-gated-correctness defect as #6 (confirmed 12 Aug; report-only, NOT yet fixed).** `.reveal { opacity: 0 }` → `.reveal.in { opacity: 1 }` (`globals.css:529-539`), with `.in` added only when the `IntersectionObserver` fires (`Reveal.tsx:30`). If the observer never fires (no IO support, disconnected early, an element that never intersects), correct content sits at `opacity:0` **permanently** — the same class as the HeroGauge stall (#6), one layer over in CSS. **Fix = the same inversion:** default `.reveal` to visible and arm the hide via a JS-added class on mount, so no-JS / IO-failure leaves content on screen and the reveal plays only when JS confirms it will observe. Lower severity than #6 (missing content reads as "absent", not as a false number) but `Reveal` is a **shared primitive with broad blast radius** — its own change + visual verification, not a bundle. Most at-risk where it wraps data (card tiles, disclosures, best-cards "HOW WE RANK"). *(Also, verified 11 Aug: `Reveal` is ruled OUT as the pinned-transform cause — its transform transition animates fine; unrelated, workaround stays.)*
14. **Dead-file deletion — DONE (12 Aug).** Removed the retired auth assets (`public/videos/auth-bg{,-mobile}.{mp4,webm}`; `public/images/auth-bg.jpg`, `auth-bg-poster.jpg`, `auth-bg-mobile-poster.jpg` — all unreferenced; `/login` uses `/video/signin-bg.*`) and the now-unreachable `app/(marketing)/landing/page.tsx` (the 301 lives in `next.config.js`, so no 404). `creditiq-gold.html` was already absent (CLAUDE.md doc mention only). Left `public/videos/hero-loop.*` + `where-points.*` in place (not confirmed dead — separate check).
...
15. **Loadable now:** body renders **Geist**, not the spec's Inter. **The real mechanism is `globals.css:916`** — a universal `*, *::before, *::after { font-family: var(--font-geist-sans) … !important }` rule that forces Geist on everything (verified 11 Aug). `app/layout.tsx`'s `<body>` also sets `--font-geist-sans`, and `--font-inter` is never loaded by `next/font`. **Wiring `--font-body` to Inter does nothing until that `!important` universal rule is removed or scoped** — that's the first move, not a token edit. The general `--font-mono` is still Geist Mono, not JetBrains. Fraunces-300 axis is already added (`globals.css:5`).
    ...
    - **Separability verdict (12 Aug): NOT separable → MOVED to the Batch 3/4 gold page walk.** The `[data-ciq]` font declarations (`globals.css:1483–1486`: Clash Display, Instrument Serif, Space Mono, Space Grotesk) are all **non-`!important`**, so the `:916` universal `!important` rule **already overrides them** — the gold pages render Geist today, not their declared fonts. Removing/scoping `:916` to let Inter/Fraunces render would **reactivate** those retired gold fonts on the four still-gold pages (`/feed`, `/pro`, `/profile`, `/my-cards`). `:916` can therefore only be neutralized *together with* deleting the `[data-ciq]` font block — i.e. as part of migrating those four pages. **Font debt is not a standalone step; it lands in the Batch 3/4 gold walk.**
...
### The walk (grouped by route-group)

> ⚠ **The 2 Aug palette audit is OBSOLETE — do not scope any page from its hex counts.** Confirmed stale on **both Spend and Travel** (13 Aug): those pages were tokenised *after* 2 Aug, so the audit's raw-hex counts and cream/slate grounds no longer match live code. Spend was ~95% done (only exact-match shadow literals left); Travel was fully done (cream survives only as dead `var(--token,#cream)` fallbacks that render white). **Re-scan every remaining page against live code before scoping its visit** — grep raw hex vs `var(--…,#…)` fallbacks first. Third stale-doc finding this week (after `STATUS-10-AUG` and the Batch-0 diagnoses).
...
| **Spend ✅ (13 Aug)** | `/spend-optimizer`, `/points-optimizer` | **DONE (live-code, not audit).** Already tokenised; slate `#f1f5f9` ground already white; only exact-match shadow literals left → `rgba(20,35,53)→--ink`, `rgba(140,95,18)→--copper`. Kept: `#B84230` warning red (pending canonical call), amber info tint, white-on-dark. Radius deferred to the global pass. `/optimize` (dark/gold) stays in the gold walk, not here. |
| **Travel ✅ (13 Aug)** | `/travel`, `/lounge-tracker`, `/trip-planner`, `/transfer-partners`, `/sweet-spots` (whole `(travelgrp)` folder = the unit) | **DONE (live-code).** Already tokenised; **NO raw cream** — cream survives only as dead `var(--token,#cream)` fallbacks that render white. Remaining raw hex are intentional keeps: bank-brand colours (transfer-partners), semantic status colours (trip-planner greens/reds), per-category accents (sweet-spots), white-on-dark. Only real swap: `/trip-planner` `rgba(140,95,18)→--copper` ×7. Radius deferred; `/trip-planner` content simplification still Batch 6. |
| ~~standalone~~ | `/statement-truth` (recheck live), ~~`/transfer-partners`~~ (done in Travel folder) | `/statement-truth` lives in `(wallet)` — re-scan against live code before its visit (audit is stale). |
...
**Parked from the wallet-CRUD cluster (`feature/cards-crud-and-profile`) — cross-repo / IA, NOT in that branch:**
33. **WhatsApp provenance label rename — lives in gogo-memory-os (separate repo/deploy).** The web renamed the user-points badge "Estimated" → "Self-entered" (commit `371120d4`). The bot's user-facing label is rendered in the AskGogo repo, not creditIQ — `app/api/wa/portfolio/route.ts` only returns a `verified` boolean. Until the bot repo renames it, WhatsApp says "Estimated" while the web says "Self-entered": the **same** assistant-vs-web provenance gap the rag.ts rule 7 fix just closed, reopened on a third surface. Related to #29 (WA honesty leak) but distinct — this is the label word, not the fallback-path leak.
