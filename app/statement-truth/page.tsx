            ...
            {step === 'result' && report && (
              <>
                ...
                <div style={{ background: 'var(--ink,#142950)', borderRadius: 18, padding: '20px 24px', marginBottom: 20 }}>
                  <div style={{ fontFamily: 'var(--font-mono,monospace)', fontSize: 9, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase' as const, color: 'var(--copper-3,#D89B2A)', marginBottom: 8 }}>EARNED THIS MONTH VS TOTAL BALANCE</div>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 10 }}>
                    <span style={{ fontSize: 32, fontWeight: 800, color: 'var(--copper-3,#D89B2A)' }}>+{(report.totalRewardsEarned||0).toLocaleString('en-IN')}</span>
                    <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)' }}>pts earned this billing cycle</span>
                  </div>
                  <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.50)', marginBottom: 12, lineHeight: 1.5 }}>Add your total balance (check HDFC SmartBuy or app) to use in Trip Planner:</p>
                  {balanceSaved ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontSize: 22, fontWeight: 800, color: 'var(--copper-3,#D89B2A)' }}>{parseInt(totalBalance.replace(/,/g,'')).toLocaleString('en-IN')} pts total</span>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      ...
                      <button onClick={() => { if (totalBalance) { localStorage.setItem('hdfc_total_points', totalBalance); setBalanceSaved(true); } }}
                        ...
                        style={{ padding: '11px 18px', background: 'var(--copper-3,#D89B2A)', color: '#fff', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' as const, opacity: totalBalance ? 1 : 0.5 }}>
                        Save to wallet
                      </button>
                    </div>
                  )}
