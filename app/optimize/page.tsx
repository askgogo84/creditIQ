          ...
          <div className="flex flex-col lg:grid lg:grid-cols-[400px,1fr] gap-6">
            <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
              <div className="bg-ink-900/40 border border-white/10 rounded-xl p-4 space-y-5">
                <div>
                  <label className="text-[10px] font-mono uppercase tracking-widest text-ink-400 mb-1.5 block">Select your card</label>
                  <select
                ...
                <button onClick={fetchAIAdvice} disabled={aiLoading} className="w-full btn-primary" style={{ opacity: aiLoading ? 0.6 : 1 }}>
                  {aiLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Strategizing...</> : <><Sparkles className="w-4 h-4" /> Get AI strategy</>}
                </button>
              </div>
              <div className="hidden sm:flex justify-center">
                <div style={{ width: '80%', maxWidth: 280 }}><CardMockup card={card} size="md" /></div>
              </div>
            </aside>
            <div className="space-y-4 min-w-0">
              <div className="bg-ink-900/40 border border-white/10 rounded-xl p-4">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-3 mb-4">
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-widest text-ink-400">Best redemption</div>
                    <div className="font-display text-3xl sm:text-4xl text-emerald-300 tabular glow-emerald">{formatINR(bestValue)}</div>
                    <div className="text-xs text-ink-400 mt-0.5 truncate max-w-[200px]">{recommendations[0]?.option.partner ?? recommendations[0]?.option.type}</div>
                  </div>
