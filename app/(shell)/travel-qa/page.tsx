...
function emptyResult(): Result {
  return { status: 'idle', cashRows: 0, awardRows: 0, walletRoutes: 0, programmes: [], message: 'Not run' }
}
...
export default function TravelQaPage() {
  ...
  async function runCase(test: TravelCoverageCase) {
    ...
    try {
      const response = await authedFetch('/api/flights/fusion', {
        method: 'POST',
      ...
      const programmes = [...new Set(rows.map((row: any) => row.award?.program).filter(Boolean))] as string[]
      const status: Result['status'] = cashRows > 0 && awardRows > 0 ? 'pass' : cashRows > 0 || awardRows > 0 ? 'partial' : 'fail'
      const message = status === 'pass'
        ? 'Cash + award inventory returned'
        : status === 'partial'
          ? cashRows > 0 ? 'Cash only — award coverage missing' : 'Award only — cash coverage missing'
  ...
  return (
    <main style={{ width: 'min(calc(100% - 48px), 1180px)', margin: '0 auto', padding: '32px 0 80px', color: 'var(--ink)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'end', gap: 20, marginBottom: 18 }}>
        <div>
          ...
          <p style={{ margin: 0, color: 'var(--ink-2)', fontSize: 13 }}>Run live cash + award + wallet fusion route by route before final UX polish.</p>
        </div>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 14 }}>
        <Metric label="Run" value={summary.run} />
        <Metric label="Pass" value={summary.pass} />
      ...
      {(['domestic', 'international'] as const).map(region => (
        <section key={region} style={{ marginTop: 24 }}>
          ...
          <div style={{ overflow: 'hidden', border: '1px solid var(--line)', borderRadius: 12, background: 'var(--surface)' }}>
            <div style={headerGridStyle}><span>Route</span><span>Cabin</span><span>Cash</span><span>Awards</span><span>Wallet</span><span>Programmes</span><span>Status</span><span /></div>
      ...
      <p style={{ marginTop: 14, color: 'var(--ink-3)', fontSize: 10, lineHeight: 1.5 }}>PASS = cash and award rows both returned. PARTIAL is deliberately visible: award-only is still valuable, but it means cash matching needs work; cash-only means award coverage needs work. Wallet routes are counted separately.</p>
    </main>
  )
}
...
function Status({ result }: { result: Result }) {
  const color = result.status === 'pass' ? 'var(--green)' : result.status === 'fail' ? 'var(--red)' : result.status === 'partial' ? 'var(--amber)' : 'var(--ink-3)'
...
const headerGridStyle = { display: 'grid', gridTemplateColumns: '1.25fr .6fr .45fr .45fr .45fr 1.4fr .8fr .45fr', gap: 10, alignItems: 'center', minHeight: 38, padding: '0 12px', background: 'var(--surface-2)', color: 'var(--ink-3)', fontSize: 9, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.05em' } as const
const rowGridStyle = { display: 'grid', gridTemplateColumns: '1.25fr .6fr .45fr .45fr .45fr 1.4fr .8fr .45fr', gap: 10, alignItems: 'center', minHeight: 54, padding: '7px 12px', borderTop: '1px solid var(--line)', fontSize: 10 } as const
