          ...
          <div className="shell">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.max(selected.length + (selected.length < 4 ? 1 : 0), 2)}, 1fr)`, gap: 12, marginBottom: 24 }} className="grid-2-mobile">
              {selectedCards.map(card => {
                ...
                return (
                  <div key={card.id} style={{ background: 'var(--paper,#FAF5EB)', border: '2px solid var(--copper-3,#D89B2A)', borderRadius: 18, overflow: 'hidden', boxShadow: '0 4px 20px rgba(212,163,115,0.12)' }}>
            ...
            {/* Picker */}
            {showPicker && (
              <div style={{ background: 'var(--paper,#FAF5EB)', border: '1px solid var(--line,rgba(20,41,80,0.08))', borderRadius: 18, padding: 20, marginBottom: 24, boxShadow: '0 8px 32px rgba(20,41,80,0.08)' }}>
            ...
            {selected.length === 0 && (
              <div style={{ textAlign: 'center', padding: '56px 20px', background: 'var(--paper,#FAF5EB)', borderRadius: 20, border: '1px solid var(--line,rgba(20,41,80,0.08))' }}>
                <div style={{ fontFamily: 'var(--font-serif,Georgia,serif)', fontSize: 48, color: 'var(--copper-3,#D89B2A)', fontStyle: 'italic', marginBottom: 16 }}>=</div>
                <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--ink,#142950)', marginBottom: 8 }}>No cards selected yet</h2>
                <p style={{ fontSize: 14, color: 'var(--ink-3,#5A6A8A)', marginBottom: 24 }}>Add cards using the slot above, or browse the full catalog.</p>
                <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
                  ...
                  <Link href="/cards" style={{ padding: '12px 24px', background: 'transparent', color: 'var(--ink,#142950)', border: '1.5px solid var(--ink,#142950)', borderRadius: 12, fontSize: 14, fontWeight: 600, textDecoration: 'none', display: 'inline-block' }}>Browse all 93 cards →</Link>
                </div>
              </div>
            )}
          </div>
