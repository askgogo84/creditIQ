...
function programmesForCard(cardId: string | null) {
  if (!cardId) return [] as string[]
  const rails = [...railsForCard(cardId, 'flight'), ...railsForCard(cardId, 'hotel')]
...
function insightText(row: any) {
  return [
    row.title,
    row.content,
    ...(Array.isArray(row.card_mentions) ? row.card_mentions : []),
    ...(Array.isArray(row.bank_mentions) ? row.bank_mentions : []),
  ].filter(Boolean).join(' ')
}
...
export function matchInsightToWallet(row: any, wallet: WalletIdentity[]): WalletInsightMatch {
  ...
  const matchedCards = wallet
    .filter(card => mentions.some((mention: string) => looseMatch(mention, card.name, 6)) || looseMatch(text, card.name, 7))
    .map(card => card.name)
  const matchedBanks = wallet
    .filter(card => bankMentions.some((mention: string) => looseMatch(String(mention).replace(/bank$/i, ''), String(card.bank).replace(/bank$/i, ''), 4)))
    .map(card => card.bank)

  const programmeMatches = new Set<string>()
  for (const card of wallet) {
    for (const programme of programmesForCard(card.cardId)) {
      if (looseMatch(text, programme, 5)) programmeMatches.add(programme)
    }
  }
  ...
  const walletBoost = uniqueCards.length
    ? 90 + Math.min(18, (uniqueCards.length - 1) * 6)
  ...
  const relevanceReason = uniqueCards.length
    ...
    : matchedProgrammes.length
      ? `Why you’re seeing this: a card in your wallet can reach ${matchedProgrammes[0]}`
      : uniqueBanks.length
        ? `Why you’re seeing this: you hold a ${uniqueBanks[0]} card`
        : null

  return {
    score,
    matchedCards: uniqueCards,
    matchedBanks: uniqueBanks,
    matchedProgrammes,
    relevanceReason,
...
export async function rankedWalletIntelligence(sb: any, userId: string, limit = 100) {
  ...
  const ranked = (intel.data ?? [])
    .map((row: any) => ({ row, match: matchInsightToWallet(row, wallet) }))
    .sort((a: any, b: any) => {
...
export async function syncWalletIntelligenceNotifications(sb: any, userId: string, maxCreate = 12) {
  ...
  const { data: existing } = await sb.from('user_notifications')
    .select('source_ref')
    .eq('user_id', userId)
  ...
  const inserts = candidates
    .filter((item: any) => !seen.has(String(item.row.id)))
    .map((item: any) => ({
      user_id: userId,
      ...
      source_ref: String(item.row.id),
      metadata: {
        insight_type: item.row.insight_type,
        matched_cards: item.match.matchedCards,
        matched_programmes: item.match.matchedProgrammes,
        relevance_reason: item.match.relevanceReason,
      },
    }))
