...
export function resolveEligible(booking: Booking, basis: EligibleBasis): number {
  switch (basis.basis) {
    case 'ROOM_ONLY': return booking.roomOnlyMinor;
    case 'ROOM_PLUS_TAX':
      if (booking.roomPlusTaxMinor === undefined) throw new Error('ROOM_PLUS_TAX eligibility requires roomPlusTaxMinor');
      return booking.roomPlusTaxMinor;
    case 'TOTAL': return booking.grossMinor;
  }
}

function transferExactComputable(ctx: EngineContext): boolean {
  return ctx.route.status === 'ACTIVE' && ctx.ratio !== null && ctx.minTransfer !== null && ctx.transferIncrement !== null && !ctx.ratioConflict;
}
...
function makeCashCandidate(ctx: EngineContext): RedemptionCandidate {
  ...
  const stranded = ctx.rules.mechanic === 'CASH_OFFSET' && permittedMin !== null && prog > 0 && prog < permittedMin ? prog : 0;
  return {
    kind: 'CASH', mechanic: null, programmePointsSpent: 0, existingProgrammePointsConsumed: 0,
    programmePointsReceived: 0, residualProgrammeBalance: prog, strandedResidualProgrammePoints: stranded,
    bankPointsRequiredMinimum: 0, bankPointsToTransferExact: null, bankPointsRetained: ctx.bank.points,
    offsetMinor: null, awardTaxesMinor: null, benchmarkCashFareMinor: ctx.booking.cashFareMinor ?? null,
    benchmarkState: ctx.booking.cashFareMinor !== undefined ? (ctx.booking.cashFareState ?? 'CAPTURED') : null,
    feeMinor: 0, cashPayableMinor: ctx.booking.grossMinor,
    incrementalBookingOffsetPerTransferredBankPointPaise: null, cashAvoidedPerTransferredBankPointPaise: null,
    marginalRateVsPreviousCandidate: null, instructionBlocked: null, durationHours: null, irreversible: false,
...
function makePortalCandidate(ctx: EngineContext): RedemptionCandidate | null {
  ...
  const offsetMinor = safeMultiply('portal offset', pointsUsable, portal.value_paise_per_point);
  const cashBeforeFee = safeSubtract('portal cash before fee', booking.grossMinor, offsetMinor);
  const cashPayableMinor = safeAdd('portal cash payable', cashBeforeFee, feeMinor);
  const prog = ctx.programmeBalance?.points ?? 0;
  const permittedMin = ctx.permitted?.min ?? null;
  const stranded = ctx.rules.mechanic === 'CASH_OFFSET' && permittedMin !== null && prog > 0 && prog < permittedMin ? prog : 0;

  return {
    kind: 'PORTAL', mechanic: 'CASH_OFFSET', programmePointsSpent: 0, existingProgrammePointsConsumed: 0,
    programmePointsReceived: 0, residualProgrammeBalance: prog, strandedResidualProgrammePoints: stranded,
    bankPointsRequiredMinimum: pointsUsable, bankPointsToTransferExact: null,
    bankPointsRetained: safeSubtract('portal bank retained', bank.points, pointsUsable), offsetMinor,
    awardTaxesMinor: null, benchmarkCashFareMinor: null, benchmarkState: null, feeMinor, cashPayableMinor,
    incrementalBookingOffsetPerTransferredBankPointPaise: null, cashAvoidedPerTransferredBankPointPaise: null,
    marginalRateVsPreviousCandidate: null, instructionBlocked: null, durationHours: null, irreversible: false,
    provenance: [...ctx.provenance],
  };
...
function effectiveBankRequirement(spend: number, held: number, ctx: EngineContext): number | null {
...
export function findPermittedRedemptions(ctx: EngineContext): GenerationResult {
  ...
  if (rules.pricing === 'FIXED_VALUE') {
    ...
    if (ctx.quoteMinorPerBaseUnit == null || ctx.permitted == null) return { candidates, eliminated, balanceState: 'BELOW_MINIMUM', legalSpendSet: [], quoteRequired: false };
    legal = legalCashOffsetSet(ctx);
  } else {
    mechanic = 'AWARD_PRICE';
    let entryPoints: number | null = null;
    if (rules.pricing === 'PUBLISHED_CHART') {
      const match = rules.award_chart.value.entries.find((entry) => entry.zone_id === ctx.booking.zoneId && entry.cabin === ctx.booking.cabin && entry.fare_tier === ctx.booking.fareTier);
      if (match) { entryPoints = match.points; awardTaxesMinor = match.taxes_minor ?? null; }
    } else if (rules.quote) {
  ...
  for (const spend of [...legal].sort((a, b) => a - b)) {
    ...
    const shortfall = safeSubtract('programme shortfall', spend, fromExisting);
    if (shortfall > 0 && ctx.route.status !== 'ACTIVE') { eliminated.push({ reason: 'TRANSFER_UNAVAILABLE', wouldHaveSpent: spend }); continue; }
    if (shortfall > 0 && ctx.ratioConflict) { eliminated.push({ reason: 'RATIO_CONFLICT', wouldHaveSpent: spend }); continue; }
    if (shortfall > 0 && ctx.ratio === null) { eliminated.push({ reason: 'RULE_UNKNOWN', wouldHaveSpent: spend }); continue; }

    const bankRequired = shortfall === 0 ? 0 : bankPointsForProgramme(shortfall, ctx.ratio!);
    if (bankRequired > ctx.bank.points) { eliminated.push({ reason: 'UNAFFORDABLE', wouldHaveSpent: spend }); continue; }
    ...
    if (mechanic === 'CASH_OFFSET') {
      offsetMinor = offsetPaise(spend, ctx.fixedValue!.amount_minor, ctx.fixedValue!.points, ctx.quoteMinorPerBaseUnit!, BASE_MINOR_PER_BASE_UNIT);
      cashPayableMinor = safeSubtract('programme cash payable', ctx.booking.grossMinor, offsetMinor);
    } else {
    ...
    candidates.push({
      kind: 'PROGRAMME', mechanic, programmePointsSpent: spend, existingProgrammePointsConsumed: fromExisting,
      programmePointsReceived: received, residualProgrammeBalance: residual, strandedResidualProgrammePoints: stranded,
      bankPointsRequiredMinimum: bankRequired, bankPointsToTransferExact: exact,
      bankPointsRetained: safeSubtract('programme bank retained', ctx.bank.points, sent), offsetMinor,
      awardTaxesMinor: candidateAwardTaxes, benchmarkCashFareMinor: candidateBenchmark, benchmarkState,
      feeMinor: 0, cashPayableMinor,
      incrementalBookingOffsetPerTransferredBankPointPaise: null, cashAvoidedPerTransferredBankPointPaise: null,
      marginalRateVsPreviousCandidate: null, instructionBlocked, durationHours: bankRequired > 0 ? ctx.durationHours : null,
      irreversible: bankRequired > 0, provenance: [...ctx.provenance],
    });
  }

  return { candidates, eliminated, balanceState: computeBalanceState(legal, affordableAll, affordableBankOnly), legalSpendSet: legal, quoteRequired };
}
