...
export function cardToText(card: any): string {
  ...
  if (card.category_rewards?.length > 0) {
    const catRewards = card.category_rewards
      .map((cr: any) =>
        cr.category + ': ' + cr.rate + (cr.unit === 'percent' ? '%' : 'x') +
        (cr.cap_inr_monthly ? ' (cap Rs.' + cr.cap_inr_monthly + '/mo)' : '')
      )
      .join(', ')
  ...
  if (card.redemption_options?.length > 0) {
    const redStr = card.redemption_options
      .map((r: any) =>
        r.type + (r.partner ? ' (' + r.partner + ')' : '') + ': Rs.' + r.value_per_point_inr + '/point'
      )
      .join(', ')
  ...
  if (card.devaluations?.length > 0) {
    const recent = card.devaluations
      .slice(0, 3)
      .map((d: any) => d.date + ': ' + d.description + ' (' + d.impact + ' impact)')
      .join('; ')
...
export async function retrieveRelevantCards(
  ...
  options: {
    ...
    maxFee?: number
    intent?: 'travel' | 'cashback' | 'dining' | 'fuel' | 'shopping' | 'general'
  } = {}
): Promise<{ cards: CreditCard[]; context: string; devaluations: string }> {
  ...
  const scored = allCards.map(card => {
    ...
    if (intent === 'cashback') {
      if (card.category?.includes('cashback')) score += 30
      if (card.reward_currency === 'cashback') score += 20
    }
    if (intent === 'dining') {
      if (card.category_rewards?.some((cr: any) => cr.category === 'dining')) score += 25
    }
    if (intent === 'fuel') {
      if ((card as any).fuel_surcharge_waiver) score += 20
      if (card.category_rewards?.some((cr: any) => cr.category === 'fuel')) score += 20
    }
    if (intent === 'shopping') {
      if (card.category?.includes('shopping')) score += 25
      if (card.category_rewards?.some((cr: any) => ['online', 'amazon', 'flipkart'].includes(cr.category))) score += 20
    }

    const keywords = queryLower.split(' ').filter((w: string) => w.length > 3)
    for (const kw of keywords) {
      if (cardText.includes(kw)) score += 5
    }

    if (maxFee !== undefined && card.annual_fee_inr > maxFee) score = -100

    for (const cat of spendCategories) {
      if (card.category_rewards?.some((cr: any) => cr.category === cat)) score += 15
    }
  ...
  const devContext =
    devaluationEvents.length > 0
      ? devaluationEvents
          .slice(0, 10)
          .map((d: any) =>
            (d.card_name ?? d.card_id) + ': ' + d.description + ' (' + d.event_date + ', ' + d.impact + ' impact)'
          )
          .join('\n')
      : allCards
          .filter(c => c.devaluations && c.devaluations.length > 0)
          .flatMap(c =>
            (c.devaluations ?? []).slice(0, 2).map(d => c.name + ': ' + d.description + ' (' + d.date + ')')
          )
          .slice(0, 10)
          .join('\n')

  return { cards: finalCards, context, devaluations: devContext }
}

export function buildRagSystemPrompt(context: string, devaluations: string): string {
