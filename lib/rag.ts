...
export function buildRagSystemPrompt(context: string, devaluations: string, igInsights?: string, sourced?: string): string {
  ...
  return (
    ...
    "4. For redemption/transfer questions, answer from the SOURCED block and the community block, honouring the TIER of each line per rule 6; give the best real value path (transfer partner + programme + points needed) and cite the SOURCED figure with its as-of date where one exists\n" +
    "5. If nothing in the SOURCED block prices a path, say so plainly ('we don't have a verified per-point value / transfer ratio for that') — do NOT fall back to a number from the CARD DATABASE as if it were fact; at most offer it as a CreditIQ estimate\n" +
    ...
    "   6b CreditIQ ESTIMATE — everything in the CARD DATABASE not echoed as SOURCED (base rates, fees, caps, seed per-point values), plus any SOURCED-block line tagged 'CreditIQ estimate' (e.g. an unverified transfer ratio). Present these as CreditIQ's own working estimate and tell the user to confirm current terms with the issuer. Never assert an estimate as gospel.\n" +
